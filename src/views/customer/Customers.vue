                 ...
                 <div class="export-table">
                    <vs-table :data="datas" search pagination max-items="5">
                        ...
                        <vs-tr :data="tr" :key="tr.id" v-for="(tr, indextr) in data">
                            ...
                            <vs-td>{{ data[indextr].customer_name   }}</vs-td>
...
export default {
  ...
  data() {
    return {
      ...
      cellAutoWidth: true,
      selectedFormat: "xlsx",
      headerTitle: ["Id",  "Customer Name","Gender","Phone Number","Address","Email","Status"],
      headerVal: ["id", "customer_name", "gender", "phone_number","address","email","status"],
      datas: [],
      activePrompt: false,
    }
  },
  methods: {
    ...
    async DeleteData(id,notif) {
        this.$store.dispatch("DELETE_CUSTOMER", id).then(data => {
            this.datas = data;
