...
<section id="card-navigation">
                ...
                <!-- SLOT = ACTION -->
                 ...
                 <div class="export-table">
                    <vs-table :data="datas" search pagination max-items="5">
                        ...
                        <vs-th sort-key="transaction_date"> Transaction Date   </vs-th>
                        ...
                        <vs-tr :data="tr" :key="tr.id" v-for="(tr, indextr) in data">
                            ...
                            <vs-td>{{ data[indextr].id_customer_name   }}</vs-td>
                            <vs-td>{{ data[indextr].transaction_date   }}</vs-td>
    ...
    <vs-popup fullscreen  class="holamundo" :title="titlepopup" :active.sync="popupActiveAdd">
...
export default {
  ...
  data() {
    return {
      ...
      cellAutoWidth: true,
      selectedFormat: "xlsx",
      headerTitle: ["Id", "Role", "Order Name","Gender","Phone Number","Address","Email","Status"],
      headerVal: ["id", "id_customer_name", "transaction_date", "gender", "phone_number","address","email","status"],
      datas: [],
      activePrompt: false,
    }
  },
  methods: {
    getOrder() {
      this.$store.dispatch("TRANSACTION").then(data => {
        this.datas = data;
      });
    },

    async DeleteData(id,notif) {
        this.$store.dispatch("DELETE_TRANSACTION", id).then(data => {
            this.datas = data;
