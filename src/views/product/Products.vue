                ...
                <!-- SLOT = ACTION -->
                 ...
                 <div class="export-table">
                    <vs-table :data="datas" search pagination max-items="5">
                        ...
                        <vs-th sort-key="photo"> Photo   </vs-th>
                        <vs-th sort-key="stock"> Stock   </vs-th>
                        <vs-th sort-key="description"> Description   </vs-th>
                        <vs-th sort-key="price"> Price   </vs-th>
                        <vs-th sort-key="weight"> Weight   </vs-th>
                        <vs-th sort-key="long"> Long   </vs-th>
                        <vs-th sort-key="heigt"> Heigt   </vs-th>
                        <vs-th sort-key="wide"> Wide   </vs-th>
                        ...
                        <vs-tr :data="tr" :key="tr.id" v-for="(tr, indextr) in data">
                            ...
                            <!-- <vs-td>{{ data[indextr].role_name   }}</vs-td> -->
                            <vs-td>{{ data[indextr].product_name   }}</vs-td>
                            <vs-td>
								<img :src="'/images/upload_product/' + data[indextr].photo" class="mt-5 rounded-full " width="100px" height="100px">
							</vs-td>
                            <vs-td>{{ data[indextr].stock   }}</vs-td>
                            <vs-td>{{ data[indextr].description   }}</vs-td>
                            <vs-td>{{ data[indextr].price   }}</vs-td>
                            <vs-td>{{ data[indextr].weight   }}</vs-td>
                            <vs-td>{{ data[indextr].long   }}</vs-td>
                            <vs-td>{{ data[indextr].heigt   }}</vs-td>
                            <vs-td>{{ data[indextr].wide   }}</vs-td>
                            <vs-td><vs-chip :color="data[indextr].status == 1 ? 'primary' : 'danger'">{{ data[indextr].status == 1 ? "Active" : "No Active" }}</vs-chip></vs-td>
                        </vs-tr>
                        </template>
...
export default {
  ...
  data() {
    return {
      ...
      cellAutoWidth: true,
      selectedFormat: "xlsx",
      headerTitle: ["Id", "Category Name", "Product Name","photo","stock","description","price","weight", "long","heigt","wide","status"],
      headerVal: ["id", "id_category", "product_name", "photo", "stock", "description", "price", "weight","long","heigt", "wide","status"],
      datas: [],
      activePrompt: false,
    }
  },
  methods: {
    ...
    async DeleteData(id,notif) {
        this.$store.dispatch("DELETE_PRODUCT", id).then(data => {
            this.datas = data;
