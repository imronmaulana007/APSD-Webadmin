    ...
    <div>
        <div class="vx-row mb-6">
            ...
            <div class="vx-col sm:w-2/3 w-full">
                <vs-input size="large" v-model="role_name" class="mt-5 w-full" name="Role Name" v-if="detail != true" />
                <vs-input size="large" v-model="role_name" class="mt-5 w-full" disabled name="Role Name" v-else/>
            </div>
        </div>
        <div class="vx-row mb-6">
            <div class="vx-col sm:w-1/3 w-full">
...
export default {
  ...
  watch: {
      data(val){
        ...
        if(Object.entries(this.data).length === 0) {
            ...
            let { id, role_name, status } = JSON.parse(JSON.stringify(this.data))
            this.dataId = id
  ...
  methods: {
    ...
    getRole() {
      this.$store.dispatch("ROLE").then(data => {
        this.datas = data;
