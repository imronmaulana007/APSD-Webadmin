                ...
                <!-- SLOT = ACTION -->
                 ...
                 <div class="export-table">
                    <vs-table :data="datas" search pagination max-items="5">
                        ...
                        <vs-th sort-key="admin_name"> Admin Name   </vs-th>
                        <vs-th sort-key="gender"> Gender   </vs-th>
                        <vs-th sort-key="phone_number"> Phone Number   </vs-th>
                        ...
                        <vs-tr :data="tr" :key="tr.id" v-for="(tr, indextr) in data">
                            ...
                            <vs-td>{{ data[indextr].id_role   }}</vs-td>
                            <!-- <vs-td>{{ data[indextr].role_name   }}</vs-td> -->
                            <vs-td>{{ data[indextr].admin_name   }}</vs-td>
                            <vs-td>{{ data[indextr].gender == 1 ? "Male" : "Female" }}</vs-td>
                            <vs-td>{{ data[indextr].phone_number   }}</vs-td>
...
export default {
  ...
  data() {
    return {
      ...
      cellAutoWidth: true,
      selectedFormat: "xlsx",
      headerTitle: ["Id", "Role", "Admin Name","Gender","Phone Number","Address","Email","Status"],
      headerVal: ["id", "id_role", "admin_name", "gender", "phone_number","address","email","status"],
      datas: [],
      activePrompt: false,
    }
  },
  methods: {
    ...
    async DeleteData(id,notif) {
        this.$store.dispatch("DELETE_ADMIN", id).then(data => {
            this.datas = data;
